...
# 聚合快讯/早晚报专栏标题特征(36氪「9点1氪」、新浪「AI热点小时报/实时…热点速递」、晨间快讯、早报/晚报…)。
# 这类是把当天 SpaceX/胖东来/FIFA 等大杂烩塞一篇,即便捎带点了 AI 公司名也非单条 AI 新闻。
# Run#19 教训:它们靠"正文含具名实体"路绕过 AI 闸,需在标题层面直接挡掉。
# 06-16 实测:新浪整点「小时报/热点速递」刷屏大模型类(6 条占 4,且全无图),补其措辞。
_BRIEF_RE = re.compile(
    r"\d+\s*点\s*\d+\s*氪|晨间快讯|晚间快讯|晨间播报|晚间播报|早间播报|"
    r"早报|晚报|一周(融资|大事|要闻|热点)|本周(融资|大事|要闻)|"
    r"热点小时报|热点速递", re.I)  # 收紧(M1):原 小时报/实时…速递 太宽,误杀"小时报告""实时推理速递"等正经新闻
